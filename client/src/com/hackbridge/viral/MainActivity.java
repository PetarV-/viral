...
{
    private AwarenessState getAware()
    {
        SharedPreferences sharedPref = this.getPreferences(Context.MODE_PRIVATE);
        // identity reading and init switch
        int avr = sharedPref.getInt("aware", -1);
    ...
    private void setAware(AwarenessState aware)
    {
        SharedPreferences sharedPref = this.getPreferences(Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = sharedPref.edit();
        switch (aware)
    ...
    {
        SharedPreferences sharedPref = this.getPreferences(Context.MODE_PRIVATE);
        return sharedPref.getLong("identity", -1);
    }

    private void setIdentity(long ident)
    {
        SharedPreferences sharedPref = this.getPreferences(Context.MODE_PRIVATE);
        SharedPreferences.Editor editor = sharedPref.edit();
        editor.putLong("identity", ident);
    ...
    protected void onCreate(Bundle savedInstanceState)
    {
        ...
        Log.d("LAG-GEO", "Before geo, onCreate in App..");
        Intent intent = new Intent(Settings.ACTION_LOCATION_SOURCE_SETTINGS);
        startActivity(intent);
        
        //locationManager.requestLocationUpdates(provider, 200, 1, mylistener);
        
        Log.d("LAG-GEO", "After creation geo");
    ...
    protected void onListItemClick(ListView lv, View v, int pos, long id)
    {
        ...
        while (true)
        {
            ...
            {
                Log.d("LAG-GEO", "InterruptedException? What should I do?");
            }
            if (location.getLocation() != null)
            {
                Log.d("LAG-GEO", "Latitude: " + location.getLocation().getLatitude() +
